...
target_loc = code_loc
bird_list = e2L.statusInList(
    'target-PR', bird_list, session, target_loc, target_time)

# 4. Add endemic & Introduced
bird_list = e2L.statusAvibase(
    ['endemic-PR', 'introduce-PR'], bird_list, code_loc)
...
statusTable = [['endemic-PR', 'bold'],
               ['introduce-PR', 'color-violet'], ['target-world', 'sym-*']]
...
spacing = '.95'
condition_tableau = [
    'Main table display only non-hybrid birds with occurence >0.1\\%.', " ( bird['freq']['year'] >= 0.001)"]
# condition_rare = ['\\footnotesize{>.1\\%}'," (bird['freq']['year'] < .01) and (bird['freq']['year'] > 0.001)"]
condition_rare = [
    '\\footnotesize{>.1\\%}', " (bird['freq']['year'] < .00) and (bird['freq']['year'] > 0.001)"]


# 6. Write To LateX
#print('Write to latex')
e2L.write_to_latex(projet_name, filename, bird_list, col,
                   condition_tableau, condition_rare, family, format, spacing, info)
