...
listHotspot = ['L1801824']
listHospotName = ['Ngulia Lodge']
...
spacing = '.95'
condition_tableau = [
    'Main table display only non-hybrid birds with occurence >1\\%.', " ( bird['freq']['year'] >= 0.01)"]
condition_rare = [
...
for i, h in enumerate(listHotspot):
    ...
    bird_list = e2L.statusInList(
        'target-KE', bird_list, session, "KE", target_time)
    col = []
    col.append(e2L.TableInput(info, 'checkbox', 1))
    statusTable = [['target-KE', 'bold'], ['target-world', 'sym-*']]
