...
LANGUAGE = [['Croatian', 'HR', 'Includes all species and non-species taxa recorded in Croatia. Thanks to Josip Turkalj for these translations.'],
            ...
            ['French', 'FR', 'All birds in the world. Denis Lepage, who runs the Avibase website',
...
def auth(usr, pwd):
    ...
    form_data = [('_eventId', 'submit'), ('execution', 'e1s1'),
                 ('lt', matches[1]), ('password', pwd), ('username', usr)]
...
def bird_creator(s, code_loc, lang, cat, byear, eyear, bmonth, emonth):
    ...
    assert all([c in poss_cat for c in cat]) or (
        cat in poss_cat), 'One or several categorie asked ( %s ) are not available.' % cat
    assert byear <= eyear, 'byear (%d) needs to be before eyear (%d)' % (
        byear, byear)
    ...
    bc_bird_list, info = load_barchart(
        s, code_loc, byear, eyear, bmonth, emonth)
    taxa_bird_list = load_taxa(lang, cat)
    info['lang'] = lang
    info['lang'].append('LA')

    # Replace
    samples_size_month_0 = [
        1 if x == 0 else x for x in info['samples_size']['month']]
    samples_size_season_0 = [
    ...
    for bc_bird in bc_bird_list:
        for taxa_bird in taxa_bird_list:
            if taxa_bird["sciName"] == bc_bird['sciName']:
                ...
                bird_nb_week = list(
                    map(lambda x, y: x*y, bird['freq']['week'], info['samples_size']['week']))
                bird_nb_month, bird_nb_season, bird_nb_year = week_to_else(
                    bird_nb_week)
...
def load_barchart(s, code_loc, byear, eyear, bmonth, emonth):
    ...
    for l in r.iter_lines():
        ...
        if not line:
            ...
            assert len(info['samples_size']['week']
                       ) > 0, 'Empty Barchart! Check the barchart link above, maybe there is not data for your query or the query is wrong'
            info['samples_size']['month'], info['samples_size']['season'], info['samples_size']['year'] = week_to_else(
                info['samples_size']['week'])
            ...
            assert len(
                freq) == 48, "Number of bird frequency is not equal to 48"
...
def statusInList(status, bird_list, session, target_loc, target_time):
    ...
    myList = list(map(lambda res: res.cssselect('.Heading-sub--sci')
                  [0].text_content(), tree.cssselect('.Observation-species .Heading')))
...
def statusInTarget(status, bird_list, session, code_loc, target_loc, target_time, bmonth, emonth):
    url = 'http://ebird.org/ebird/targets?r1={code_loc}&r2={target_loc}&t2={target_time}&bmo={bmonth}&emo={emonth}'.format(
        code_loc=code_loc, target_loc=target_loc, target_time=target_time, bmonth=bmonth, emonth=emonth)

    print('Load target data from '+url)
    r = session.get(url)
    tree = html.fromstring(r.content)

    target = list(map(lambda res: res.cssselect(
        '.ResultsStats-title a em')[0].text_content(), tree.find_class('ResultsStats')))
...
def write_to_latex(projname, filename, bird_list, col, condition_tableau, condition_rare, family, format, spacing, info):
    ...
    for line in f2:
        ...
        if 'newcommand{\samples_size}' in line:
            line = line[:-1] + str(round(info['samples_size']['year'])) + \
                ' checklists ('+str(info['nb_taxa'])+' differents species)} '
            ...
            for bird in bird_list:  # write Content
                ...
                if eval(condition_tableau[1]):
                    if bird['familyComName'] != family_current and family:  # family name
                        f.write(
                            '\\multicolumn{'+str(len(col))+'}{c}{\\textbf{'+bird['familyComName']+'}} \\\\ \n')
                        f.write('\\hline\n')
            ...
            if u != 0:  # check if there are rare bird
                ...
                f.write('} \n')
                f.write('\\toprule\n\\\\\n')
                f.write('\\multicolumn{'+str(2*n_rare_col) +
                        '}{c}{\\textsc{ \\Large{Rare'+condition_rare[0]+'}}} \\\\ \n')
                f.write('\\\\\n\\midrule\n')
                for cc in range(0, c):
                    for c_r in range(0, n_rare_col-1):
                        f.write(
                            '\\underline{\\hspace{3ex}} \t &' + bird_list_r[cc+c*c_r] + ' \t &')
                    f.write('\\underline{\\hspace{3ex}} \t &' +
    ...
    def __init__(self, info, type, option1=None, option2=None, option3=None):
        ...
        if self.type == 'lang':
            ...
            if self.option1 == 'year':
                ...
                season = ['Sp', 'Sm', 'F', 'W']
                self.title = season[self.option2] + '\\footnotesize{ (' + num2str(
                    info['samples_size']['season'][self.option2]) + ')} '
